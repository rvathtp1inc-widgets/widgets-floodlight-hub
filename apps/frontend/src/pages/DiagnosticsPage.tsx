...
import { useDiagnosticsCommands, useDiagnosticsEvents, useDiagnosticsHealth, useDiagnosticsTimers, useExecutionDiagnostics } from '../hooks/useDiagnostics';
...
export function DiagnosticsPage() {
  ...
  return <section className="min-w-0 space-y-4">
    ...
    {activeTab === 'Service Health' ? <HealthSummary health={healthQuery.data} recentEventsCount={eventsQuery.data?.length ?? 0} recentCommandFailuresCount={commandFailures.length} activeTimersCount={activeTimers.length} isLoading={healthQuery.isLoading} isError={healthQuery.isError} errorMessage={toErrorMessage(healthQuery.error)} onRefresh={() => void healthQuery.refetch()} isRefreshing={healthQuery.isFetching} /> : null}
  </section>;
}
