...
export function OutputsPage() {
  ...
  return <section className="space-y-4">
    ...
    <section className="rounded border border-slate-800 bg-slate-900/70 p-4">
      <div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="text-lg font-semibold text-white">Virtual Security Panel</h2><p className="text-sm text-slate-400">Active Condition → Violated zone · Inactive Condition → Normal zone</p></div><StatusBadge tone="neutral">Service status unavailable</StatusBadge></div>
      <p className="mt-2 text-xs text-slate-500">Listener and Savant client status are not exposed by the current backend API. Runtime state is memory-only and does not survive restart.</p>
